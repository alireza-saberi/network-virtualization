function Filtering(input_nodes)% change = input_nodes
...
%     index2 = Distance_Checker(Net,required_X,required_Y,required_D);
    ...
    if (isempty(index3) | index3 == -1)
        continue;
    end  
    display('AR check');
    result = ARChecker(Net,index3)
    display(sprintf('Node %d is selected from our virtual resoure to build our virtual network ',result))
    NN(kk,6)= result;
